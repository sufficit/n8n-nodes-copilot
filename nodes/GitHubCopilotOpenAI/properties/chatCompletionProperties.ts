...
// All chat-completion-specific UI properties.
// These are exported separately so that nodeProperties.ts can apply
// displayOptions.show.operation = ['chatCompletions'] to each of them.
export const CHAT_COMPLETION_PROPERTIES: INodeProperties[] = [
	// Model selection (shared component)
    ...
    "function": {
      ...
      "parameters": {
        ...
        "required": ["location"]
				...
				description:
					'Optional: Array of tools/functions available to the model (OpenAI format).',
			...
			},
			{
				...
				default: false,
				description:
					'When the primary model does not support vision, automatically use a vision-capable fallback model to process images.',
			},
			{
